...
int
stream_write(lua_State* L, int fd, const char* data, size_t datasize) {
    size_t status = 1;
...
static int
push_read_result(lua_State* L, int res, int nonblocking) {
    if (res == -1) {
        char* errmsg = NULL;
        size_t _errno;
...
#endif
        {
            ...
            size_t size = FormatMessageA(
                ...
                _errno, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)&messageBuffer, 0, NULL);
...
static int
stream_read_line(lua_State* L, int fd, int chop, int nonblocking) {
    luaL_Buffer b;
    char c = '\0';
    luaL_buffinit(L, &b);
    size_t res = 1;
...
static int
stream_read_all(lua_State* L, int fd, int nonblocking) {
    size_t res;
    luaL_Buffer b;
    luaL_buffinit(L, &b);
...
int
stream_read_bytes(lua_State* L, int fd, size_t length, int nonblocking) {
    size_t res;
    char* p;
    luaL_Buffer b;
    luaL_buffinit(L, &b);
...
int
stream_read(lua_State* L, int fd, const char* opt, int nonblocking) {
    size_t success;
    if (*opt == '*') {
        opt++; /* skip optional '*' (for compatibility) */
    }
    switch (*opt) {
        case 'l': /* line */ return stream_read_line(L, fd, 1, nonblocking);
        case 'L': /* line with end-of-line */ return stream_read_line(L, fd, 0, nonblocking);
        case 'a': return stream_read_all(L, fd, nonblocking); /* read all data available */
        default: return luaL_argerror(L, 2, "invalid format");
    }
}

int
stream_is_nonblocking(int fd) {
#ifdef _WIN32
...
int
stream_set_nonblocking(int fd, int nonblocking) {
#ifdef _WIN32
    HANDLE h = (HANDLE)_get_osfhandle(fd);
    if (h == INVALID_HANDLE_VALUE) {
        errno = EBADF;
...
int
stream_as_filestream(lua_State* L, int fd, const char* mode) {
